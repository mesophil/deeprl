...
class TinyImageNetPaths:
  ...
  def _make_paths(self, train_path, val_path, test_path,
    ...
    val_nids = os.listdir(val_path)
    for nid in val_nids:
      anno_path = os.path.join(val_path, nid, nid+'_boxes.txt')
      imgs_path = os.path.join(val_path, nid, 'images')
      label_id = self.ids.index(nid)
  ...
  def __init__(self, root_dir, mode='train', preload=True, load_transform=None,
    ...
    if self.preload:
      ...
      self.img_data = np.zeros((self.samples_num,) + self.IMAGE_SHAPE,
                               dtype=np.float32)
      self.label_data = np.zeros((self.samples_num,), dtype=np.intc)
